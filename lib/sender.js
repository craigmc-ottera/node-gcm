...
Sender.prototype.sendNoRetry = function(message, recipient, callback) {
    ...
    getRequestBody(message, recipient, function(err, body) {
        ...
        var request_options = _.defaultsDeep({
            ...
            uri: Constants.GCM_SEND_URI.replace("{PROJECT_ID}", this.options.firebaseConfig.project_id),
            json: body
        }, this.options, {
...
function getRequestBody(message, recipient, callback) {
    var legacyBody = message.toJson();
    var body = {
        message: {
            notification: legacyBody.notification
        }
    };
    body.message.data = legacyBody.data;
    if(typeof recipient == "string") {
        body.message.topic = recipient;
    ...
    if(Array.isArray(recipient)) {
        if(!recipient.length) {
            ...
            body.message.token = recipient[0];
            return nextTick(callback, null, body);
        }
